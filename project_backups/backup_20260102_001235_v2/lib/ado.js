...
const ADO = {
    ...
    getBranch: async (config, branchName) => {
        ...
        try {
            const response = await ErrorHandler.fetchWithRetry(url, ADO.getFetchOptions('GET', config.pat));
            if (!response.ok) {
                const errorMsg = ErrorHandler.getUserFriendlyError(new Error("Failed to get branch: " + response.statusText));
                throw new Error(errorMsg);
            }
            const data = await response.json();
            return data.count > 0 ? data.value[0] : null;
        } catch (error) {
            if (typeof Logger !== 'undefined') await Logger.error(`Branch check failed: ${error.message}`);
            throw error;
        }
    },
    ...
    getBranches: async (config) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to list branches (${response.status}): ${response.statusText} - ${rawMsg.substring(0, 100)}`);
            }
            const data = await response.json();
            return data.value.map(ref => ref.name.replace('refs/heads/', ''));
        } catch (error) {
            if (typeof Logger !== 'undefined') await Logger.error(`List branches failed: ${error.message}`);
            throw error;
        }
    },
    ...
    getExistingFilePaths: async (config, branchName) => {
        ...
        const paths = new Map(); // Lowercase Path -> Original Path
