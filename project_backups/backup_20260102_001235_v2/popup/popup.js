...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (isFullPage) {
        ...
        // Ensure base styles immediately to prevent flash of wrong layout
    ...
    const savedConfig = await chrome.storage.local.get([
        ...
        'gitUser', 'gitEmail', 'commitMessage', 'showLogs', 'reminderInterval'
    ]);
    ...
    // Show Logs Toggle
    const chkShowLogs = document.getElementById('chk-show-logs');
    const btnViewLogs = document.getElementById('btn-view-logs');
    const rightPanel = document.getElementById('right-panel');

    // Default to false if undefined, or use saved value
    const showLogsState = savedConfig.showLogs === true;
    chkShowLogs.checked = showLogsState;
    ...
    const handleLogVisibility = () => {
        if (isFullPage) {
            ...
            // Always hide the "View Logs" button in full page
            ...
            if (leftPanel) {
                ...
                leftPanel.style.marginBottom = '';
            }

            rightPanel.style.display = 'none';
            document.body.classList.remove('logs-visible');

            // Show/Hide Button based on checkbox
            btnViewLogs.style.display = chkShowLogs.checked ? 'inline-block' : 'none';
        }
    };
    ...
    // Auto-populate Git User Info on Launch if missing
    if (!savedConfig.gitUser || !savedConfig.gitEmail) {
        chrome.storage.local.get(['sfContext'], async (result) => {
            if (result.sfContext && result.sfContext.serverUrl && result.sfContext.sessionId) {
                try {
                    const userInfo = await Salesforce.fetchUserInfo(result.sfContext.serverUrl, result.sfContext.sessionId);
                    if (userInfo) {
                        if (!document.getElementById('git-user').value && userInfo.name) {
                            document.getElementById('git-user').value = userInfo.name;
                        }
                        if (!document.getElementById('git-email').value && userInfo.email) {
                            document.getElementById('git-email').value = userInfo.email;
                        }
                        // Save immediately so it persists
                        chrome.storage.local.set({
                            gitUser: userInfo.name,
                            gitEmail: userInfo.email
                        });
                        console.log('Auto-populated Git Identity on launch');
                    }
                } catch (e) {
                    console.warn('Failed to auto-populate Git Identity on launch', e);
                }
            }
        });
    }

    // --- Helper: Update Org Badge ---
    async function updateOrgBadge(serverUrl, sessionId) {
        const badge = document.getElementById('org-badge');
        if (!badge) return;
        ...
        if (stored.sfContext && stored.sfContext.orgName && stored.sfContext.serverUrl === serverUrl) {
            badge.textContent = stored.sfContext.orgName;
    ...
    // Initial Badge Load (Fallback for non-active-tab sessions)
    chrome.storage.local.get(['sfContext'], async (result) => {
        if (result.sfContext && result.sfContext.serverUrl && result.sfContext.sessionId) {
            await updateOrgBadge(result.sfContext.serverUrl, result.sfContext.sessionId);
        }
    });

    if (savedConfig.reminderInterval) document.getElementById('reminder-interval').value = savedConfig.reminderInterval;


    // --- JOB MANAGER (Dashboard History) ---
    const JobManager = {
        ...
        updateLastJob: async (status, message, endTime = null) => {
            ...
            if (history.length > 0) {
                ...
                try {
                    const logs = await Logger.getSessionLogs();
                    // Optimization: Truncate logs if HUGE (e.g. > 2000 lines) to avoid quota hits
                    lastJob.logs = logs.slice(-2000);
                } catch (e) { console.error("Failed to attach logs to job", e); }
    ...
    const updateStatus = async (msg, type = 'info') => {
        ...
        const levelMap = {
            ...
            'success': Logger.LEVELS.SUCCESS,
            'warning': Logger.LEVELS.WARNING,
            'error': Logger.LEVELS.ERROR
        };
        const level = levelMap[type] || Logger.LEVELS.INFO;
        await Logger.log(msg, level);

        if (Logger.DEBUG) console.log(`[${type.toUpperCase()}] ${msg}`);
    };
    ...
    const saveConfigToStorage = () => {
        const config = {
            ...
            showLogs: document.getElementById('chk-show-logs').checked,
    ...
    // Renew PAT Button Logic
    document.getElementById('btn-renew-pat').addEventListener('click', () => {
        const org = document.getElementById('ado-org').value.trim();
        let url = 'https://dev.azure.com/_usersSettings/tokens'; // Default

        if (org) {
            if (org.includes('.visualstudio.com')) {
                url = `https://${org}/_usersSettings/tokens`;
            } else {
        ...
        updateStatus('Opening Azure DevOps Token page...', 'info');
    });
...
<Package xmlns="http://soap.sforce.com/2006/04/metadata">
    ...
    // Full Page Logic
    // const urlParams = new URLSearchParams(window.location.search); // Moved to top
    // const isFullPage = urlParams.get('fullpage') === 'true'; // Moved to top

    if (isFullPage) {
        document.body.classList.add('full-page');
        document.getElementById('btn-full-page').style.display = 'none';

        // We now keep btn-view-logs visible per user request
    ...
    document.getElementById('btn-full-page').addEventListener('click', async () => {
        try {
            ...
            try {
                const userInfo = await Salesforce.fetchUserInfo(serverUrl, sessionId);
                if (userInfo) {
                    // Update DOM (if empty, or always? User said "predrafted", so let's overwrite or fill)
                    // Let's safe fill: only if empty? Or refresh? 
                    // User said "predrafted... from salesforce name", implies syncing.
                    if (userInfo.name) document.getElementById('git-user').value = userInfo.name;
                    if (userInfo.email) document.getElementById('git-email').value = userInfo.email;
                    // Save to storage immediately
                    chrome.storage.local.set({
                        gitUser: userInfo.name,
                        gitEmail: userInfo.email
                    });
                    updateStatus(`Welcome, ${userInfo.name}!`, 'success');
                }
            } catch (uErr) {
                console.warn('User info fetch failed', uErr);
            }
        ...
        btnReconnect.addEventListener('click', async () => {
            ...
            try {
                ...
                if (activeTab && activeTab.url) {
                    const url = new URL(activeTab.url);
                    if (url.hostname.includes('salesforce.com') || url.hostname.includes('force.com')) {
                        console.log('Reconnect prioritizes active tab:', url.hostname);
                        await connectToOrg(url.hostname);
                        return; // Done
                    }
                }

                // 2. Fallback: Find ALL open Salesforce tabs
    ...
    async function connectToOrg(domain) {
        ...
        try {
            ...
            // Auto-fill Identity
            try {
                const userInfo = await Salesforce.fetchUserInfo(session.serverUrl, session.sessionId);
                if (userInfo) {
                    document.getElementById('git-user').value = userInfo.name;
                    document.getElementById('git-email').value = userInfo.email;
                    updateStatus(`Connected as ${userInfo.name}`, 'success');
                } else {
    ...
    document.getElementById('btn-sync').addEventListener('click', async () => {
        ...
        try {
            ...
            const adoOrg = document.getElementById('ado-org').value;
            const adoProject = document.getElementById('ado-project').value;
            const adoPat = document.getElementById('ado-pat').value;

            // Validate branch name
            if (branchName) {
                try {
                    ErrorHandler.validateBranchName(branchName);
                } catch (e) {
                    await updateStatus(ErrorHandler.getUserFriendlyError(e), 'error');
                    return;
                }
            }
            ...
            if (!packageXml.trim()) {
                await updateStatus('Please provide package.xml content.', 'error');
                return;
            }

            try {
                ErrorHandler.validateXml(packageXml);
            } catch (e) {
                await updateStatus(ErrorHandler.getUserFriendlyError(e), 'error');
                return;
            }
            ...
            if (!adoOrg || !adoProject || !adoPat) {
                await updateStatus('Please fill all ADO configuration fields.', 'error');
                return;
            }
        } catch (e) {
            await updateStatus('Validation error: ' + e.message, 'error');
            return;
        }

        // Start Logger session
        await Logger.startSession();
        await updateStatus('🚀 Starting sync operation...', 'info');
        ...
        document.getElementById('btn-cancel-sync').style.display = 'inline-block';

        try {
            // Continue with existing auto-save
            saveConfigToStorage();
            ...
            const remindIntervalMins = parseInt(document.getElementById('reminder-interval').value) || 10;
            const remindIntervalMs = remindIntervalMins * 60 * 1000;
            ...
            while (!completed) {
                ...
                // Reminder Logic
                const elapsedForReminder = Date.now() - lastReminderTime;
                if (elapsedForReminder > remindIntervalMs) {
                    const totalElapsedMins = Math.floor((Date.now() - startTime) / 60000);
                    chrome.notifications.create({
                        type: 'basic',
                        iconUrl: '../icons/48.png',
                        title: 'Long Running Sync Warning',
                        message: `Sync has been running for ${totalElapsedMins} minutes.`
                    });
                    lastReminderTime = Date.now(); // Reset reset timer to remind again in X mins
                }
            ...
            while (!pushSuccess && retryCount <= MAX_RETRIES) {
                try {
                    ...
                    // --- SUCCESS ---
                    await Logger.endSession('success');
                    await JobManager.updateLastJob('success', 'Sync completed successfully.', new Date().toISOString());

                    chrome.notifications.create({
                        type: 'basic',
                        iconUrl: '../icons/48.png',
                        title: 'Salesforce Sync Complete',
                        message: `Successfully pushed to ${config.branch}`
                    });
                    ...
                    if (pushError.message.includes('TF401028') || pushError.message.includes('GitReferenceStaleException')) {
                        ...
                        if (retryCount <= MAX_RETRIES) {
                            ...
                            await Logger.endSession('failed');
        ...
        } catch (err) { // This catch closes the big logic try block
            console.error(err);
            ...
            await Logger.endSession('failed');
            // Determine if cancelled or error
            const status = (err.message === "Sync cancelled by user.") ? 'cancelled' : 'failed';
            await JobManager.updateLastJob(status, err.message, new Date().toISOString());
        } finally {
            // Reset UI
