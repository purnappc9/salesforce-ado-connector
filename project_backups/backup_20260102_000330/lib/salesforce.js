...
const Salesforce = {
    ...
    // Get Organization Info (Name, Sandbox Status)
    fetchOrgInfo: async (serverUrl, sessionId) => {
        const query = "SELECT Name, IsSandbox, OrganizationType FROM Organization";
        const url = `${serverUrl}/services/data/v${Salesforce.apiVersion}/query?q=${encodeURIComponent(query)}`;
        ...
        if (!response.ok) {
            console.warn("Failed to fetch org info:", response.statusText);
            return null;
        }
